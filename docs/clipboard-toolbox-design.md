...
- 管理设置项，例如保留天数、最大记录数、自定义存储目录、文本上限和敏感内容过滤。
...
- `monitor_enabled`：是否启用监听，当前保存在运行时状态。
...
- 本地设置：保留天数、最大记录数、自定义 SQLite 存储目录、单条文本上限、敏感内容过滤。
- 数据安全：默认本地存储、不上传，敏感过滤可跳过疑似 JWT/API Key/长 token。

### 17.2 仍作为后续

- 物理删除、跳过原因提示、敏感规则自定义正则。
- 图片、文件、富文本剪贴板支持。
- 收藏、标签、全局快捷键、导入导出。
