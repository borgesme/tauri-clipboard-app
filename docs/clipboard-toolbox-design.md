...
- `content`：完整剪贴板内容。
- `preview`：列表摘要，建议截断到 80 到 120 个字符。
...
- 后续可增加敏感内容过滤，例如疑似 JWT、API Key、银行卡号、长随机 token。
...
MVP 阶段可先不新增 UI 组件库，使用原生 CSS 完成布局。若后续追求更快 UI 迭代，可再评估引入组件库，避免一开始增加依赖复杂度。
...
- MVP 推荐软删除，便于减少误删风险和保留调试空间。
- 若用户明确要求“彻底删除”，可增加物理删除命令。

### 14.3 长文本处理

- 建议设置 `max_text_length`，避免误复制大文件内容导致数据库膨胀。
- 超长内容可跳过保存，或只保存摘要并提示用户。
